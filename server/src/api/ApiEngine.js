...
function sendJson(response, status, payload) {
  response.writeHead(status, {
    ...
    "Cache-Control": status >= 400 ? "no-store" : "private, no-store, max-age=0",
...
const PUBLIC_ENGINE_GET_PATHS = new Set([
  "/engine/health",
  "/engine/nowplaying",
  "/engine/now-playing",
  "/engine/player/nowplaying",
  "/engine/player/next",
  "/engine/player/status",
  "/engine/radio/mxcast/status",
  "/engine/xat/widget",
  "/engine/xat/stream",
]);

function isTrustedEngineOrigin(request) {
  const origin = String(request.headers.origin || "").trim();
  if (!origin) return true;
  if (TRUSTED_ENGINE_ORIGINS.has(origin)) return true;
...
function applyEngineCors(request, response) {
  const origin = String(request.headers.origin || "").trim();
  const trusted = isTrustedEngineOrigin(request);
...
function getEngineAdminToken(request) {
...
function getEngineAccess(request) {
  const configured = String(process.env.RADIO_ENGINE_ADMIN_TOKEN || "").trim();
  if (!configured) {
    return { ok: false, status: 503, error: "Engine admin token not configured." };
  }

  return timingSafeTokenEqual(getEngineAdminToken(request), configured)
    ? { ok: true, status: 200, error: "" }
    : { ok: false, status: 401, error: "Unauthorized." };
}

function isPublicEngineGet(pathname) {
  return PUBLIC_ENGINE_GET_PATHS.has(pathname)
    || pathname === "/engine/covers/default"
    || pathname.startsWith("/engine/covers/");
}

function requiresEngineAuth(method, pathname) {
  if (method === "GET" && isPublicEngineGet(pathname)) return false;
  return pathname.startsWith("/engine/") || pathname.startsWith("/api/radio/");
}
...
export class ApiEngine {
  ...
  async init() {
    ...
    await new Promise((resolve) => {
      this.server.listen(
        this.engine.config.apiPort,
        this.engine.config.host || "127.0.0.1",
        resolve,
      );
    });
    this.bindEvents();
    this.logger.info("api", "API iniciada.", {
      host: this.engine.config.host || "127.0.0.1",
      port: this.engine.config.apiPort,
    });
  }

  async handle(request, response) {
    if (!applyEngineCors(request, response)) {
    ...
    try {
      ...
      if (requiresEngineAuth(request.method, url.pathname)) {
        const access = getEngineAccess(request);
        if (!access.ok) {
          sendJson(response, access.status, { ok: false, error: access.error });
          return;
        }
      }
      ...
      sendJson(response, status, {
        ok: false,
        error: status === 413 ? "Payload too large." : "Internal engine error.",
      });
    }
  }
  ...
  async readBody(request) {
    ...
    for await (const chunk of request) {
      ...
      if (totalBytes > ENGINE_BODY_LIMIT_BYTES) {
  ...
  handleUpgrade(request, socket) {
    const url = new URL(request.url, `http://${request.headers.host}`);
    if (url.pathname !== "/engine/ws" || !isTrustedEngineOrigin(request)) {
      socket.destroy();
      return;
    }
