...
const INITIAL_DATA = { nowPlaying: null, status: { status: "offline" }, stream: null, widget: null, hasError: false };
...
export default function XatPreviewPage() {
  ...
  const refresh = useCallback(async (signal) => {
    try { setData(await loadXatWidgetData({ signal })); }
    catch { setData((current) => ({ ...current, hasError: true })); }
  ...
  return (
    <main className="bar-radio-page bar-xat-page">
      <header className="bar-xat-header"><span>Integração Xat</span><h1>Mini Widget Rádio IMORTAL0800</h1></header>
      {loading ? <Loading label="Carregando integração Xat" /> : (
        <div className="bar-xat-grid">
          <div className="bar-xat-preview"><MiniPlayerWidget track={track} status={data.widget?.status || data.status} stream={data.stream} /></div>
          <section className="bar-radio-card bar-xat-code">
            <div className="bar-radio-card-title"><h3>Código radiohtml5</h3></div>
            <code>{radioHtml5}</code>
            {data.hasError && <p>API offline ou parcialmente indisponível. Preview em modo seguro.</p>}
          </section>
