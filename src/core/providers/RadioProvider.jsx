...
const INITIAL_STATION = {
  name: "Rádio IMORTAL0800",
  program: "Rádio IMORTAL0800",
...
function parseSongTitle(rawTitle) {
  ...
  if (!title) return { artist: "IMORTAL0800", track: "Programação ao vivo" };
  ...
  for (const separator of [" - ", " – ", " — "]) {
    ...
    if (artist.trim() && track) return { artist: artist.trim(), track };
  }

  return { artist: "IMORTAL0800", track: title };
}
...
export function RadioProvider({ children }) {
  ...
  useEffect(() => {
    ...
    async function loadMetadata() {
      try {
        const response = await fetch(RADIO_STATS_URL, { cache: "no-store" });
        ...
        setCurrentStation((previous) => ({
          ...previous,
          name: data.serverTitle || "Rádio IMORTAL0800",
          program: data.online
        ...
        console.warn("Não foi possível atualizar os metadados da rádio:", metadataError);
        if (!active) return;
        setCurrentStation((previous) => ({
          ...previous,
          name: "Rádio IMORTAL0800",
        ...
        if (active) refreshTimer = window.setTimeout(loadMetadata, METADATA_REFRESH_INTERVAL);
      }
    }
  ...
  function handleVolumeChange(value) { setVolume(Math.min(Math.max(Number(value), 0), 100)); }

  return (
    <RadioContext.Provider value={{ playing, loading, error, currentStation, play, pause, toggle, volume, setVolume: handleVolumeChange }}>
      {children}
      <audio
        ref={audioRef}
        ...
        onError={() => { setLoading(false); setPlaying(false); setError("Conexão com a rádio indisponível."); }}
        onPause={() => setPlaying(false)}
        onPlay={() => { setError(""); setPlaying(true); setLoading(false); }}
...
export function useRadio() {
  ...
  if (!context) throw new Error("useRadio deve ser usado dentro de RadioProvider");
  return context;
}
