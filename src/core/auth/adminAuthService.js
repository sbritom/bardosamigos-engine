...
export const ADMIN_AUTH_DOMAIN = 'auth.imortal0800.local'
export const ADMIN_USERNAME_PATTERN = /^[a-z0-9._-]{3,32}$/
export const ADMIN_ROLES = Object.freeze({ ADMIN: 'admin', LOCUTOR: 'locutor' })
...
export function normalizeUsername(username) {
  return String(username || '').trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
}
...
export function usernameToAuthEmail(username) { return `${validateUsername(username)}@${ADMIN_AUTH_DOMAIN}` }
...
export function hasAllowedAdminRole(user, allowedRoles = [], options = {}) {
  ...
  return Boolean(roles.has(role) || (options.allowLegacyUserMetadata && user?.user_metadata?.is_admin === true && roles.has(ADMIN_ROLES.ADMIN)))
}

export async function getAdminSession() {
  const client = getSupabaseClient()
  if (!client) return { session: null, user: null, error: new Error(ADMIN_AUTH_ERRORS.SUPABASE_NOT_CONFIGURED) }
...
export async function getVerifiedAdminSession() {
  ...
  if (sessionError || !session?.access_token) return { session: null, user: null, error: sessionError || null }
  const { data: userData, error: userError } = await client.auth.getUser(session.access_token)
  if (userError || !userData?.user) return { session: null, user: null, error: userError || new Error(ADMIN_AUTH_ERRORS.INVALID_SESSION) }
  return { session, user: userData.user, error: null }
}

export async function getAdminAccessToken() { const { session } = await getAdminSession(); return session?.access_token || '' }
export async function getAdminAccess({ allowedRoles = [ADMIN_ROLES.ADMIN], allowLegacyUserMetadata = false, noSessionReason = ADMIN_AUTH_ERRORS.NO_SESSION } = {}) {
  ...
  if (error?.message === ADMIN_AUTH_ERRORS.SUPABASE_NOT_CONFIGURED) return { allowed: false, hasSession: false, reason: ADMIN_AUTH_ERRORS.SUPABASE_NOT_CONFIGURED, user: null, role: '' }
  if (error || !session?.access_token || !user) return { allowed: false, hasSession: false, reason: error ? ADMIN_AUTH_ERRORS.INVALID_SESSION : noSessionReason, user: null, role: '' }
  ...
  return { allowed, hasSession: true, reason: allowed ? '' : ADMIN_AUTH_ERRORS.NOT_ALLOWED, user, role }
}
...
export async function signInAdminWithUsername({ username, password, allowedRoles = [ADMIN_ROLES.ADMIN], allowLegacyUserMetadata = false } = {}) {
  const client = getSupabaseClient()
  if (!client) throw new Error(ADMIN_AUTH_ERRORS.SUPABASE_NOT_CONFIGURED)
  const email = usernameToAuthEmail(username)
  const { error } = await client.auth.signInWithPassword({ email, password: String(password || '') })
  if (error) throw new Error(ADMIN_AUTH_ERRORS.INVALID_LOGIN)
  const access = await getAdminAccess({ allowedRoles, allowLegacyUserMetadata })
  if (!access.allowed) { await client.auth.signOut(); throw new Error(access.reason || ADMIN_AUTH_ERRORS.NOT_ALLOWED) }
  return access
}

export async function signOutAdmin() { const client = getSupabaseClient(); if (client) await client.auth.signOut() }
