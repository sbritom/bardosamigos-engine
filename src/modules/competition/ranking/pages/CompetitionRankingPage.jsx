  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <div className="text-xs font-black uppercase text-[var(--gold)]">Ranking</div>
        ...
        <div className="flex gap-2">
          {['general', 'competition', 'season'].map((item) => (
            <button
              key={item}
              type="button"
              className={`rounded-lg border px-3 py-2 text-sm font-black ${scope === item ? 'border-[var(--gold)] bg-[var(--gold)] text-black' : 'border-[var(--border)] bg-[var(--card)]'}`}
      ...
      {loading ? <Loading label="Carregando ranking" /> : !ranking || items.length === 0 ? (
        ...
        <Card className="overflow-hidden rounded-[var(--radius)] border border-[var(--border)] bg-[var(--card)]">
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-[var(--border)] bg-black/30 text-[var(--secondary)]">
                <tr>
              ...
              <tbody>
                {items.map((item) => {
                  ...
                  return (
                    <tr key={item.id || item.profileId} className="border-b border-[var(--border)] last:border-0">
                      ...
                      <td className="px-4 py-3 text-[var(--gold)] font-black">{item.points}</td>
