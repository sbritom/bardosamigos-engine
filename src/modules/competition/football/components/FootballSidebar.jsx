import { ChevronDown, Heart, SlidersHorizontal, Trophy, X } from 'lucide-react'
import { FOOTBALL_COMPETITION_NAV, FOOTBALL_FILTERS } from '../constants/footballCenterConstants'
import { footballMatchBelongsToCompetition } from '../utils/footballCenterUtils'

const SIDEBAR_GROUPS = [
  { title: 'Brasil', ids: ['BSA'] },
  { title: 'Europa', ids: ['PL', 'PD', 'SA', 'BL1', 'FL1', 'CL'] },
  { title: 'America', ids: ['CLI'] },
  { title: 'Mundo', ids: ['WC'] },
]

const SIDEBAR_FILTERS = FOOTBALL_FILTERS.filter((filter) => ['live', 'today', 'favorites', 'finished'].includes(filter.id))
    ...
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-[var(--bds-space-12)] overflow-y-auto p-[var(--bds-space-10)] [scrollbar-color:color-mix(in_srgb,var(--bds-color-primary-hover)_45%,transparent)_transparent] [scrollbar-width:thin]" aria-label="Navegacao de futebol">
        ...
        <SidebarSection title="Filtros" icon={SlidersHorizontal}>
          <div className="space-y-[var(--bds-space-3)]">
            {SIDEBAR_FILTERS.map(({ id, label, icon: Icon }) => (
              <SidebarButton key={id} active={activeFilter === id} icon={Icon} onClick={() => onFilter(id)}>{label}</SidebarButton>
            ))}
          </div>
        </SidebarSection>
      </nav>
