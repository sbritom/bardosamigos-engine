...
const DETAILS = {
  ...
  'ranking-de-barcoins-proxima-edicao': {
    title: 'Ranking de BarCoins — Próxima edição', banner: rankingBanner, status: 'upcoming', period: 'Data a definir', prizes: ['1º 5.000 xats', '2º 2.000 xats', '3º 1.000 xats'], howToParticipate: 'Colete BarCoins participando das atividades do Bar dos Amigos durante o periodo do evento.', rules: RANKING_RULES, rankingTitle: 'Ranking Oficial', ranking: 'O Ranking de BarCoins sera contabilizado automaticamente pelo EVOX Bot.',
  },
  'bingo-do-bar-dos-amigos': {
    title: 'Bingo do Bar dos Amigos', banner: bingoBanner, status: 'active', period: 'Quartas-feiras ou fins de semana • horário divulgado no dia', prizes: ['Days', 'Xats', 'Powers'], howToParticipate: 'Aguarde o inicio do bingo e siga as instrucoes da equipe.', rules: BINGO_RULES,
  },
}
...
function statusOf(event = {}) { const status = String(event.status || '').toLowerCase(); if (['archived', 'ended', 'finished', 'closed'].includes(status)) return 'ended'; if (status === 'draft') return 'upcoming'; return 'active' }
function detailOf(event = {}) {
  const preset = DETAILS[event.slug] || {}; const recurrence = getEventRecurrenceLabel(event); const time = getEventTimeLabel(event)
  return { id: event.id, slug: event.slug, title: preset.title || event.title || 'Evento', description: getEventSummary(event) || event.description || null, banner: preset.banner || event.banner || event.metadata?.banner || null, status: preset.status || statusOf(event), period: preset.period || [recurrence, time].filter(Boolean).join(' - ') || 'Periodo a definir', howToParticipate: preset.howToParticipate || getParticipationRule(event) || 'Acompanhe as instrucoes da equipe.', prizes: preset.prizes || 'Premiacao sera informada pela equipe.', rules: preset.rules || ['Respeite as orientacoes da equipe.'], rankingTitle: preset.rankingTitle || null, ranking: preset.ranking || null, type: getEventType(event) }
}

function CompactEventList({ events, onSelect, emptyTitle }) {
  if (!events.length) return <WorkspaceEmptyState title={emptyTitle} />
  return <div className="bds-events-compact-list">{events.slice(0, 6).map((event) => { const detail = detailOf(event); return <button key={event.id || event.slug} type="button" onClick={() => onSelect(event)} className="bds-events-compact-item"><span><strong>{detail.title}</strong><small>{detail.period}</small></span><span className={`bds-events-status bds-events-status--${detail.status}`}>{detail.status === 'ended' ? 'Encerrado' : detail.status === 'upcoming' ? 'Em breve' : 'Ativo'}</span></button> })}</div>
}

function Regulations({ events, onSelect }) {
  return <div className="bds-events-regulations">{events.map((event) => { const detail = detailOf(event); return <button key={event.id || event.slug} type="button" className="bds-events-regulation-card" onClick={() => onSelect(event)}><FileText size={18} aria-hidden="true" /><span><strong>{detail.title}</strong><small>{detail.rules.length} regras principais</small></span></button> })}</div>
}

export default function EventsPage() {
  const [remote, setRemote] = useState([]); const [loading, setLoading] = useState(true); const [activeView, setActiveView] = useState(null); const [selected, setSelected] = useState(null)
  ...
  const events = useMemo(() => { const cleaned = remote.filter((event) => !isRanking(event) && !isBingo(event)); return [BINGO_EVENT, NEXT_RANKING, JULY_RANKING, ...cleaned] }, [remote])
  const bingo = events.find((event) => event.slug === BINGO_EVENT.slug); const activeEvents = events.filter((event) => statusOf(event) === 'active'); const upcomingEvents = events.filter((event) => statusOf(event) === 'upcoming'); const endedEvents = events.filter((event) => statusOf(event) === 'ended'); const selectedDetail = selected ? detailOf(selected) : null
  function openEvent(event) { setSelected(event); setActiveView(null) }
  function openView(view) { setSelected(null); setActiveView((current) => current === view ? null : view) }
  ...
  return <main className="bds-events-page bds-events-page--clean"><header className="bds-events-clean-header"><div><span>Agenda do Bar</span><h1>Eventos do Bar</h1><p>{HERO_TEXT}</p></div><div className="bds-events-header-stats" aria-label="Resumo dos eventos"><span><PartyPopper size={15} /> {activeEvents.length} ativo{activeEvents.length === 1 ? '' : 's'}</span><span><Trophy size={15} /> {upcomingEvents.length} próximo{upcomingEvents.length === 1 ? '' : 's'}</span></div></header>{loading ? <WorkspaceSkeleton rows={4} /> : <>{bingo ? <EventHighlightBanner highlight={HIGHLIGHT} event={bingo} onOpen={openEvent} /> : null}<nav className="bds-events-quick-nav" aria-label="Navegação de eventos">{quickActions.map(({ id, label, count, icon: Icon }) => <button key={id} type="button" className={activeView === id ? 'is-active' : ''} onClick={() => openView(id)}><Icon size={17} aria-hidden="true" /><span>{label}</span><strong>{count}</strong></button>)}</nav>{(activeView || selectedDetail) ? <section className="bds-events-dynamic-panel"><div className="bds-events-dynamic-panel__header"><strong>{selectedDetail ? selectedDetail.title : quickActions.find((item) => item.id === activeView)?.label}</strong>{activeView ? <Button variant="secondary" onClick={() => setActiveView(null)}>Fechar</Button> : null}</div>{renderPanel()}</section> : null}</>}</main>
}
